"""Fixtures for Collector integration tests against a live SR Linux node (T015).
...
Then run::
    ...
    uv run pytest tests/integration/test_collector/ -m integration
...
@pytest.fixture
def srlinux_collector(srlinux_reachable):
    """Yield a GnmiCollector pointed at a live SR Linux node; skip if unreachable."""
...
@pytest.fixture
def srlinux_device(srlinux_reachable):
    """Return a Device descriptor for the live SR Linux node; skip if unreachable."""
    if not srlinux_reachable:
        pytest.skip(f"SR Linux not reachable at {_host()}:{_port()}")
